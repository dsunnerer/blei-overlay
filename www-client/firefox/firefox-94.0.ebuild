...
src_prepare() {
	...
	for p in $(cat "${FILESDIR}/blei-patchset-$(ver_cut 1)"/series);do
		patch --dry-run --silent -p1 -i "${FILESDIR}/blei-patchset-$(ver_cut 1)"/$p 2>/dev/null
		if [ $? -eq 0 ]; then
			eapply "${FILESDIR}/blei-patchset-$(ver_cut 1)"/$p;
			einfo +++++++++++++++++++++++++;
			einfo Patch $p is APPLIED;
			einfo +++++++++++++++++++++++++
		else
...
src_install() {
	...
	#######
	if [ -f "${FILESDIR}"/opensuse-kde-$(ver_cut 1)/kde.js ]; then
