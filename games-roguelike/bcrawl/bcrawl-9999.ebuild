...
HOMEPAGE="https://github.com/b-crawl/bcrawl/"
SLOT="9999"
MY_P="bcrawl-9999/crawl-ref"

EGIT_REPO_URI="https://github.com/b-crawl/bcrawl.git"

SRC_URI="
	https://dev.gentoo.org/~stasibear/distfiles/stone-soup.png -> ${PN}-git.png
	https://dev.gentoo.org/~stasibear/distfiles/stone-soup.svg -> ${PN}-git.svg
"
...
src_prepare() {
	...
	sed -i -e "s/GAME = crawl$/GAME = bcrawl-git/" "${S}/Makefile" ||
		die "Couldn't append slot to executable name"

	# File required for a _pre build
	if ! [ -f "${S}/util/release_ver" ]; then
		echo "0.9999" >"${S}/util/release_ver" || die "Couldn't write release_ver"
	fi
...
src_compile() {
	...
	if use ncurses || (use !ncurses && use !tiles); then
		...
		use tiles && { mv "bcrawl-git" "${WORKDIR}"/bcrawl-ncurses-git || die; }
	fi
...
src_install() {
	...
	[[ -e "${WORKDIR}/bcrawl-ncurses-git" ]] && dobin "${WORKDIR}/bcrawl-ncurses-git"

	# don't relocate docs, needed at runtime
	rm -rf "${D}/usr/share/${PN}-git"/docs/license

	mv "${WORKDIR}/${MY_P}"/docs/crawl.6 "${WORKDIR}/${MY_P}/docs/bcrawl-git.6" ||
		die "Couldn't append slot to man page name"
	doman "${WORKDIR}/${MY_P}/docs/bcrawl-git.6"

	# icons and menu for graphical build
	if use tiles; then
		doicon -s 48 "${DISTDIR}"/${PN}-git.png
		doicon -s scalable "${DISTDIR}"/${PN}-git.svg
		make_desktop_entry "bcrawl-git" "bcrawl-git" "bcrawl-git"
	fi
}
...
pkg_postinst() {
	...
	elog "bcrawl is a slotted install that supports having"
	elog "multiple versions installed.  The binary has the"
	elog "slot appended, e.g. 'bcrawl-"git"'."

	if use tiles && use ncurses; then
		elog
		elog "Since you have enabled both tiles and ncurses frontends"
		elog "the ncurses binary is called 'bcrawl-ncurses-"git"' and the"
		elog "tiles binary is called 'bcrawl-"git"'."
	fi
}
